...
use Config::IniFiles;
use FindBin;
use lib $FindBin::Bin."/model/schema+agents/lib";
...
if(scalar(@ARGV)>=2) {
								...
								foreach my $p_primary (@{$p_primary_anal->{$analDomain}}) {
									...
									if(!defined($reason) && scalar(@localFiles) > 0) {
										...
										if($singleFile) {
											...
											if($singleFile =~ /\.gz$/) {
												$f_mode = '-|';
												push(@f_params,BP::Loader::Agents::GUNZIP,'-c',$singleFile);
											} else {
												$f_mode = '<';
	...
	print STDERR "Usage: $0 [-t|-tt] [-s|-ss] [-b] iniFile cachingDir [",join('|',sort(keys(%SECDOMAIN2EXPANAL))),"]\n"
}
