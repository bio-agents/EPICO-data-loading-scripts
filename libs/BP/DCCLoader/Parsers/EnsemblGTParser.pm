...
sub parseEnsemblGenesAndTranscripts($$$$$$$$$$) {
	...
	if(open(my $SEQREG,'-|',BP::Loader::Agents::GUNZIP,'-c',$localSeqRegion)) {
		my %config = (
	...
	if(open(my $ENSG,'-|',BP::Loader::Agents::GUNZIP,'-c',$localGenes)) {
		my %config = (
...
#			TabParser::TAG_NEG_FILTER => [[1 => 'LRG_gene']],
	...
	my %transcriptsMap;
	if(open(my $ENST,'-|',BP::Loader::Agents::GUNZIP,'-c',$localTranscripts)) {
		my %config = (
...
#			TabParser::TAG_NEG_FILTER => [[8 => 'LRG_gene']],
	...
	if(open(my $ET,'-|',BP::Loader::Agents::GUNZIP,'-c',$localExonTranscripts)) {
		my %config = (
	...
	if(open(my $ENSE,'-|',BP::Loader::Agents::GUNZIP,'-c',$localExons)) {
		my %config = (
...
#			TabParser::TAG_NEG_FILTER => [[8 => 'LRG_gene']],
	...
	my %externalDB = ();
	if(open(my $ExtDB,'-|',BP::Loader::Agents::GUNZIP,'-c',$localExternalDB)) {
		my %config = (
	...
	if(open(my $XREF,'-|',BP::Loader::Agents::GUNZIP,'-c',$localXref)) {
		my %config = (
